...
def user_ids_from_designated_channel(start_date, end_date, channel):
    deviceids_createtime = list(mongodb.logs.find({"type": "app_install_channel", "extra.installChannel": channel, "create_time": {'$gte': start_date, '$lte': end_date}}, {"username": 1, "create_time":1}))
    user_createtime_map = {}
    for item in deviceids_createtime:
        username = item['username']
        if username not in user_createtime_map.keys():
            user_createtime_map[username] = item['create_time']
    result = {}
